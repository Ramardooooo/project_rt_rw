...
<header class="bg-white shadow-sm border-b border-gray-200 ml-64 transition-all duration-300" id="mainHeader">
    <div class="flex justify-between items-center px-8 py-5">
        ...
        <div class="flex items-center space-x-3">
            <!-- Notifications Ketua -->
            <div class="relative">
                <button id="notifBtnKetua" class="p-2.5 text-gray-500 hover:text-gray-900 rounded-full hover:bg-gray-100 transition-all duration-200" onclick="if(typeof toggleNotifications === 'function') toggleNotifications('Ketua')">
                ...
                <div id="notifDropdownKetua" class="hidden absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-2xl border ring-1 ring-black/5 z-[9999] overflow-hidden">
                    <div class="p-4 border-b bg-gradient-to-r from-emerald-50 to-teal-50">
                        <div class="flex items-center justify-between">
                            ...
                            <button onclick="if(typeof markAllRead === 'function') markAllRead('ketua')" class="text-xs text-emerald-600 hover:text-emerald-800 font-medium hover:underline">Tandai semua</button>
                        </div>
                    </div>
...
function toggleSidebar() {
    ...
    if (header) header.classList.toggle('ml-64', sidebarCollapsed).classList.toggle('ml-16', !sidebarCollapsed);
    if (mainContent) mainContent.classList.toggle('ml-64', sidebarCollapsed).classList.toggle('ml-16', !sidebarCollapsed);
}
</script>
</body>
</html>  
