...
$where_clause = $search ? "LEFT JOIN warga w ON rt.ketua_rt_id = w.id WHERE rt.nama_rt LIKE '%$search%' OR rt.ketua_rt LIKE '%$search%' OR w.nama LIKE '%$search%'" : "LEFT JOIN warga w ON rt.ketua_rt_id = w.id";
$total_rt = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(rt.id) as total FROM rt $where_clause"))['total'];
$total_pages = max(1, ceil($total_rt / $limit));

$rt = mysqli_query($conn, "SELECT rt.*, w.id as warga_id, w.nama as ketua_nama, w.nik FROM rt $where_clause LIMIT $limit OFFSET $offset");
?>
...
$photo_src = !empty($r['profile_photo']) 
    ? '/PROJECT/' . $r['profile_photo']
    : null;
?>
...
<?php else: ?>
    <div class="w-10 h-10 rounded-full shadow-lg mr-3 border-2 border-gray-200 
...
<?php endif; ?>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div class="flex space-x-1">
                            ...
                            <form method="POST" class="inline" style="display:inline;">
                                <input type="hidden" name="rt_id" value="<?php echo $r['id']; ?>">
                                <button type="submit" name="delete_rt" class="bg-red-500 hover:bg-red-600 text-white px-2 py-1 rounded text-xs flex items-center" onclick="return confirm('Hapus RT ini?')" title="Delete">
