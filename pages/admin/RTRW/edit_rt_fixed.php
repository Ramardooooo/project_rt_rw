...
if (isset($_POST['update_rt'])) {
    ...
    $old_rt = mysqli_fetch_assoc(mysqli_query($conn, "SELECT nama_rt, ketua_rt_id, status FROM rt WHERE id = $rt_id"));

    $stmt = mysqli_prepare($conn, "UPDATE rt SET nama_rt=?, ketua_rt_id=?, status=? WHERE id=?");
    mysqli_stmt_bind_param($stmt, "sisi", $nama_rt, $ketua_rt_id, $status, $rt_id);
    ...
    $new_value = json_encode(['nama_rt' => $nama_rt, 'ketua_rt_id' => $ketua_rt_id, 'status' => $status]);
    ...
    <div class="max-w-xl w-full bg-white/90 backdrop-blur-md rounded-2xl shadow-lg p-7 border border-white/20 hover:shadow-2xl hover:bg-white/95 transition-all duration-300">
        ...
        <form method="POST" class="space-y-4">
            ...
            <div>
                ...
                <select name="ketua_rt_id" required class="w-full border rounded px-3 py-2 focus:outline-none focus:border-green-500">
                    ...
                    $selected = (isset($rt['ketua_rt_id']) && $rt['ketua_rt_id'] == $w['id']) ? 'selected' : '';
                    ?>
